            ...
            <OkNg
              ok={
                ...
                meta.commercialUsage === "personalNonProfit"
                  ? false
                  : meta.commercialUsage === "personalProfit"
                    ? "conditional"
...
const Unknown = () => {
  const t = useTranslation("common");
  return <span className="text-gray-300">{t("vrmLicense/unknown")}</span>;
};

const None = () => {
  const t = useTranslation("common");
  return <span className="text-gray-300">{t("vrmLicense/none")}</span>;
};
...
const Heading = memo(function Heading({ children }: { children: ReactNode }) {
  return <h1 className="mt-6 mb-4 font-bold">{children}</h1>;
});

const TableLike = memo(function TableLike({
  children,
}: {
  children: ReactNode;
}) {
  return (
    <div className="grid grid-cols-[1fr_1fr] gap-y-1 gap-x-2">{children}</div>
  );
});

const Row = memo(function Row({ children }: { children: ReactNode }) {
  return (
    <div className="contents border-b border-b-black last:border-0">
      {children}
    </div>
  );
});

const Cell = memo(function Cell({ children }: { children: ReactNode }) {
  return <div className="py-1 leading-snug break-all">{children}</div>;
});
